'''
Find in js-scripts of discord some emoji by name (for example slightly_smiling_face)
Copy this code (last time it was string for JSON.parse)
Write to console in useful view:

em = JSON.parse(...);
s = ['emojis = {'];
for (let key in em){
        s.push(`\t# region ${key}`);
        em[key].forEach(function(item){
                item.names.forEach(function(name){
                        s.push(`\t\t'${name}': '${item.surrogates}',`);
                });
        });
        s.push(`\t# endregion`);
}
